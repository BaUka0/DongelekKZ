...
from .utils import get_bot_response_from_lmstudio
...
def get_response(request):
    try:
        ...
        bot_response_text = get_bot_response_from_lmstudio(user_input, history_queryset)
        ...
        print(f"Ошибка в get_response для {request.user}: {e}")
        return HttpResponseServerError(json.dumps({"error": "Внутренняя ошибка сервера."}), content_type='application/json')
...
def voice_input(request):
    ...
    with sr.Microphone() as source:
        try:
            ...
            text = recognizer.recognize_google(audio, language="kk-KZ")  # Распознавание на русском
            return JsonResponse({"success": True, "text": text})
        except sr.UnknownValueError:
            return JsonResponse({"success": False, "error": "Не удалось распознать голос"})
        except sr.RequestError as e:
            return JsonResponse({"success": False, "error": f"Ошибка сервиса: {e}"})